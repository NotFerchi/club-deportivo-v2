...
const registrarAsistenciaSocio = async (client, sesionId, socioId) => {
  const inscripcion = await client.query(
    `SELECT inscripcion_id FROM inscripciones_clases
     WHERE sesion_id = $1 AND socio_id = $2 AND estado = 'Confirmada'`,
    [sesionId, socioId]
  );

  if (inscripcion.rows.length === 0) {
    const error = new Error('El socio no está inscrito en esta sesión');
  ...
  const duplicado = await client.query(
    `SELECT asistencia_id FROM asistencia
     WHERE sesion_id = $1 AND socio_id = $2 AND fecha = CURRENT_DATE`,
    [sesionId, socioId]
  );
  ...
  registrarAsistenciaQr: async (req, res) => {
    ...
    try {
      ...
      if (payload.type === 'socio') {
        ...
        resultado = await registrarAsistenciaSocio(client, sesionId, socioId);
      } else {
