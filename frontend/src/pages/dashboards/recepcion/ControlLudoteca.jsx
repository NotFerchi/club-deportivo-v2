  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.6)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
      ...
      <div id="ticket-imprimible" style={{ background: 'white', borderRadius: '20px', width: '100%', maxWidth: '400px', boxShadow: '0 24px 64px rgba(0,0,0,0.35)', overflow: 'hidden' }}>
        ...
        {/* Header sin emoji — iniciales del niño */}
        ...
        <div style={{ padding: '1.5rem' }}>
          <div style={{ marginBottom: '1rem', display: 'flex', flexDirection: 'column', gap: '6px' }}>
            {[
              { icono: <Baby size={13} color="#3b82f6" />,  label: 'Nino',        value: ticket.nombre_hijo },
          ...
          <div style={{ background: 'linear-gradient(135deg, #eff6ff, #dbeafe)', border: '1.5px solid #bfdbfe', borderRadius: '14px', padding: '1rem', marginBottom: '1rem' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <span style={{ fontSize: '12px', color: '#3b82f6', fontWeight: 700, display: 'flex', alignItems: 'center', gap: '4px' }}><Clock size={13} /> Hora limite</span>
          ...
          <div style={{ background: '#fefce8', border: '1px solid #fde68a', borderRadius: '10px', padding: '10px 14px', marginBottom: '1.25rem', fontSize: '11px', color: '#92400e', textAlign: 'center', fontWeight: 600, lineHeight: 1.6, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px' }}>
            ...
            Tiempo maximo de estancia: <strong>2 horas</strong>. Pasado este tiempo se aplicara una sancion.
          </div>
      ...
      <div style={{ background: 'white', borderRadius: '20px', padding: '1.75rem', width: '100%', maxWidth: '480px', boxShadow: '0 24px 64px rgba(0,0,0,0.3)', maxHeight: '90vh', overflowY: 'auto' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
          <div>
            ...
            <p style={{ margin: '3px 0 0', fontSize: '12px', color: '#64748b' }}>Completa los datos del nino</p>
          </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          ...
          <div>
            <label style={labelStyle}>Nombre del nino *</label>
            <input type="text" value={nombreHijo} onChange={e => setNombreHijo(e.target.value)} placeholder="Nombre completo del nino" style={inputStyle} />
          </div>
        ...
        <div style={{ display: 'flex', gap: '0.75rem', marginTop: '1.5rem' }}>
          ...
          <button onClick={handleSubmit} disabled={!puedeEnviar || cargando} style={{ flex: 2, background: puedeEnviar && !cargando ? 'linear-gradient(135deg, #2563eb, #3b82f6)' : '#cbd5e1', color: puedeEnviar ? 'white' : '#94a3b8', border: 'none', borderRadius: '10px', padding: '11px', fontSize: '13px', fontWeight: 700, cursor: puedeEnviar && !cargando ? 'pointer' : 'not-allowed', transition: 'all 0.2s', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px' }}>
            {cargando ? <><Loader2 size={14} style={{ animation: 'spin 1s linear infinite' }} /> Registrando...</> : <><CheckCircle size={14} /> Registrar Entrada</>}
...
function SeccionHistorial() {
  ...
  const filtrados = historial.filter(r => {
    const texto = `${r.nombre_hijo || ''} ${r.nombre_nino || ''} ${r.tutor_nombre || ''} ${r.nombres || ''} ${r.apellido_paterno || ''}`.toLowerCase();
    ...
    <div style={{ marginTop: '1.5rem', border: '1px solid #e2e8f0', borderRadius: '14px', overflow: 'hidden' }}>
      ...
      <button onClick={toggleAbierto} style={{ width: '100%', padding: '0.9rem 1.25rem', background: abierto ? '#f8fafc' : 'white', border: 'none', cursor: 'pointer', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.6rem' }}>
          ...
          <span style={{ fontWeight: 700, fontSize: '13px', color: '#1e293b' }}>Historial de los ultimos 7 dias</span>
          {historial.length > 0 && (
        ...
        <div style={{ borderTop: '1px solid #f1f5f9' }}>
          ...
          <div style={{ padding: '0.75rem 1.25rem', borderBottom: '1px solid #f1f5f9' }}>
            <div style={{ position: 'relative' }}>
              ...
              <input
                type="text"
                placeholder="Buscar por nombre de nino o tutor..."
                value={busqueda}
          ...
          {loading ? (
            ...
            <div style={{ padding: '2rem', textAlign: 'center', color: '#94a3b8', fontSize: '12px' }}>
              {busqueda ? 'Sin resultados para esa busqueda' : 'Sin registros en los ultimos 7 dias'}
            </div>
          ) : (
            <div style={{ maxHeight: '340px', overflowY: 'auto' }}>
              {filtrados.map((r, i) => {
                const nombre    = r.nombre_hijo || r.nombre_nino || '—';
                const tutor     = r.tutor_nombre || `${r.nombres || ''} ${r.apellido_paterno || ''}`.trim() || '—';
              ...
              {detalle && (
                <div style={{ margin: '0.5rem 1.25rem 0.75rem', padding: '0.75rem 1rem', background: '#f8fafc', borderRadius: '10px', border: '1px solid #e2e8f0', fontSize: '12px' }}>
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.5rem', marginBottom: detalle.observaciones ? '0.5rem' : 0 }}>
                    <div><span style={{ color: '#94a3b8', fontSize: '10px', fontWeight: 700, textTransform: 'uppercase' }}>Entrada</span><div style={{ fontWeight: 700, color: '#1e293b' }}>{formatFechaHora(detalle.hora_entrada_local || detalle.hora_entrada)}</div></div>
                    <div><span style={{ color: '#94a3b8', fontSize: '10px', fontWeight: 700, textTransform: 'uppercase' }}>Salida</span><div style={{ fontWeight: 700, color: '#1e293b' }}>{detalle.hora_salida ? formatFechaHora(detalle.hora_salida_local || detalle.hora_salida) : '—'}</div></div>
                  </div>
    ...
    <div className="chart-box">
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.25rem', flexWrap: 'wrap', gap: '1rem' }}>
        <div>
          ...
          <p style={{ margin: '3px 0 0', fontSize: '12px', color: '#64748b' }}>Rango de edad: 3-7 anos · Limite: 2 horas</p>
        </div>
      ...
      {ninos.length === 0 ? (
        <div style={{ textAlign: 'center', padding: '3.5rem 2rem', background: '#f8fafc', borderRadius: '14px', border: '2px dashed #e2e8f0' }}>
          ...
          <p style={{ color: '#475569', margin: 0, fontWeight: 700, fontSize: '14px' }}>No hay ninos en ludoteca</p>
